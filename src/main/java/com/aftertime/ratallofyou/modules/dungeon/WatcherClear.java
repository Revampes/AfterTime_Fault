...
public class WatcherClear {
    ...
    public void onChat(ClientChatReceivedEvent event) {
        if (!ModConfig.enableWatcherClear || done || bloodOpen || event == null || event.message == null) return;
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (!ModConfig.enableWatcherClear || event.phase != TickEvent.Phase.START) return;
    ...
    private void showKillMobsMessage() {
        ...
        DungeonUtils.sendTitle(EnumChatFormatting.RED.toString() + EnumChatFormatting.BOLD + "Kill Blood Mobs", "", 0, 100, 0);
