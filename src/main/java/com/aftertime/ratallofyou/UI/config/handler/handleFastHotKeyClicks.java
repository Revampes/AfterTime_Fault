...
public class handleFastHotKeyClicks {
    ...
    public void handleFastHotKeyClicks(int mouseX, int mouseY, int mouseButton) {
        ...
        if (clickInRight) {
            ...
            List<com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyEntry> list = FastHotkeyStore.getInstance().getActiveEntries();
            for (int i = 0; i < gui.fastRows.size(); i++) {
                ...
                if (mouseX >= removeX && mouseX <= removeX + Dimensions.FH_REMOVE_WIDTH && mouseY >= removeY && mouseY <= removeY + Dimensions.FH_REMOVE_HEIGHT) {
                    if (i < list.size()) {
                        // Remove entry i and reindex
                        List<com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyEntry> rebuilt = new ArrayList<>();
                        for (int j = 0; j < list.size(); j++) { if (j == i) continue; rebuilt.add(new com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyEntry(list.get(j).label, list.get(j).command, rebuilt.size())); }
                        FastHotkeyStore.getInstance().replaceActiveEntries(rebuilt);
                        ModConfigIO.saveFhkPresets(FastHotkeyStore.getInstance().getPresetsView(), FastHotkeyStore.getInstance().getActiveIndex());
            ...
            if (mouseX >= x && mouseX <= x + addW && mouseY >= addY && mouseY <= addY + Dimensions.FH_ADD_HEIGHT) {
                // Append new empty entry
                List<com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyEntry> rebuilt = new ArrayList<>(list);
                rebuilt.add(new com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyEntry("", "", rebuilt.size()));
                FastHotkeyStore.getInstance().replaceActiveEntries(rebuilt);
                ModConfigIO.saveFhkPresets(FastHotkeyStore.getInstance().getPresetsView(), FastHotkeyStore.getInstance().getActiveIndex());
                gui.rebuildFastHotkeyRowsForDetail();
            }
        ...
        if (clickInLeft) {
            ...
            int y = leftContentY + 12; if (gui.fhkPresetNameInput != null) {
                ...
                int btnX = leftX + leftW - 60; if (mouseX >= btnX && mouseX <= btnX + 60 && mouseY >= y && mouseY <= y + 16) {
                    String name = gui.fhkPresetNameInput.text.trim(); if (!name.isEmpty()) {
                        ...
                        boolean exists = false; for (com.aftertime.ratallofyou.UI.config.ConfigData.FastHotkeyPreset p : FastHotkeyStore.getInstance().getPresetsView()) { if (p.name.equalsIgnoreCase(name)) { exists = true; break; } }
                        if (!exists) {
                            int idx = FastHotkeyStore.getInstance().addPreset(name);
                            gui.fhkSelectedPreset = idx; gui.fhkPresetNameInput.text = ""; gui.rebuildFastHotkeyRowsForDetail();
                        }
                    }
                    return;
                }
            ...
            for (int i = 0; i < presets.size(); i++) {
                ...
                if (mouseX >= openX && mouseX <= openX + openW && mouseY >= openY && mouseY <= openY + presetBtnH) { FastHotkeyStore.getInstance().setActiveIndex(i); gui.fhkSelectedPreset = i; gui.rebuildFastHotkeyRowsForDetail(); return; }
            ...
            int yToggle = leftContentY + 12 + 22 + 12 + (presets.size() * (presetBtnH + 4)) + 6;
